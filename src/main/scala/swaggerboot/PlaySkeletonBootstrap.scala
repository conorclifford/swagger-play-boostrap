...
object PlaySkeletonBootstrap extends App {
  ...
  if (!(args.length == 2 || (args.length == 3 && args(2) == "-client") || args.length == 4 && Set(args(2), args(3)) == Set("-client", "-replace"))) {
    System.err.println(
    """
      |Usage: PlaySkeletonBootstrap <swagger-spec> <output-directory> [-client] [-overwrite]
  ...
  val replace = !genServerCode && args.length == 4
